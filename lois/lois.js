...
(() => {
  ...
  function showTooltip() {
    ...
    // Start fade out at 1.0s, fully gone by ~1.2s (200ms transition)
    hideTimer1 = setTimeout(() => {
      tip.classList.remove("is-visible");
    }, 1000);

    hideTimer2 = setTimeout(() => {
      // no-op; ensures timing aligns with the 1.2s requirement
    }, 3000);
  }
  ...
  btn.addEventListener("click", async () => {
    try {
      ...
      // If clipboard is blocked, still show feedback (user can retry)
      showTooltip();
    }
  });
})();
